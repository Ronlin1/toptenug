...
export function RankingRow({ result }: { result: RankingResult }) {
  ...
  return (
    <div className="ranking-row">
      ...
      <div><a className="person-name" href={`/people/${result.slug}`}>{result.name}</a><div className="person-meta">{result.provenance_count} public evidence signals</div><ShareRanking resultId={result.result_id} entityName={result.name} /></div>
