...
type Point = { category_slug:string; quarter:string; rank:number; score:number; factor_coverage:number };

export function RankHistoryChart({ points }: { points: Point[] }) {
  const navigate = (point: Point) => {
    window.location.href = `/technology/${point.category_slug}?quarter=${encodeURIComponent(point.quarter)}`;
  };
  return <section className="chart-wrap">
    ...
    <table className="sr-table"><thead><tr><th>Quarter</th><th>Rank</th><th>Score</th></tr></thead><tbody>{points.map(point => <tr key={`${point.category_slug}-${point.quarter}`}><td>{point.quarter}</td><td>#{point.rank}</td><td>{point.score.toFixed(1)}</td></tr>)}</tbody></table>
  </section>;
}
