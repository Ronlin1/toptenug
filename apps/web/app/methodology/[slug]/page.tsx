...
export default async function MethodologyPage({ params }: { params: Promise<{slug:string}> }) {
  ...
  return <main><div className="breadcrumb"><a href="/">TopTenUG</a> / Methodology</div>
    ...
    <table className="method-table"><thead><tr><th>Factor</th><th>Weight</th><th>Metric</th><th>Normalization</th></tr></thead><tbody>{Object.entries(data.factors).map(([name,f]) => <tr key={name}><td>{name.replaceAll("_"," ")}</td><td>{Math.round(f.weight*100)}%</td><td><code>{f.metric}</code></td><td>{f.normalization}</td></tr>)}</tbody></table>
